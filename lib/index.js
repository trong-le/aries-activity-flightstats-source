...
export default class FlightStatsSource extends Activity {
    ...
    async getTaxiData({ airport, embark, date, appId, currentTaxi, hourOffset, dayOffset, appKey, utc }) {
        ...
        if (!currentTaxi) {
            if (hour < 12) {
                 taxiHour = hour + hourOffset;
                 taxiDay = day - dayOffset;
            } else {
                taxiHour = hour - hourOffset;
            }
        }

        const path = `flightstatus/rest/v2/json/airport/status/${airport}/${embark}/${year}/${month}/${taxiDay}/${taxiHour}`;
        const uri = `${baseURI}${path}`;
        const params = {
            uri: uri,
            qs: { appId, appKey, utc },
