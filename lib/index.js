...
export default class FlightStatsSource extends Activity {
	...
	async fidsArrival(config) {
		const uri = `${baseURI}fids/rest/v1/json/CVG/arrivals?appId=${config.appId}&appKey={config.appKey}&requestedFields=scheduledTime%2CscheduledDate%2Cflight%2CstatusCode%2Cremarks%2CactualTime%2CactualGateTime%2CcodesharesAsNames%2CairlineName%2CflightId&timeWindowBegin=720&timeWindowEnd=720&lateMinutes=15&useRunwayTimes=true&excludeCargoOnlyFlights=false`
		const params = {
			uri: uri,
			qs: config,
			json: true
