...
export default class FlightStatsSource extends Activity {
	...
	async onTask(activityTask, config) {
		...
		switch (config.method) {
			case 'departures':
			const departures = await this.getFlightTracksDeparturesByAirport(config);
			data = departures.flightTracks;
			break;
			case 'arrivals':
			const arrivals = await this.getFlightTracksArrivalsByAirport(config);
			data = arrivals.flightTracks;
			break;
			case 'taxi':
			const taxi = await this.getFlightStatusArrivalsByAirport(config);
