...
export default class FlightStatsSource extends Activity {
	...
	async onTask(activityTask, config) {
		...
		// Create generator for past 6 hours of taxi data 6
		return function*() {
			for loop
				yield results
			end
	...
	async fidsArrival(config) {
		const uri = `${baseURI}fids/rest/v1/json/CVG/arrivals?appId=${config.appId}&appKey={config.appKey}&requestedFields=scheduledTime%2C+scheduledDate%2C+flight%2CstatusCode%2C+remarks%2CactualTime%2CactualGateTime%2CcodesharesAsNames%2C+airlineName%2CflightId&timeWindowBegin=720&timeWindowEnd=720&lateMinutes=15&useRunwayTimes=true&excludeCargoOnlyFlights=false`
		const params = {
			uri: uri,
			qs: config,
			json: true
