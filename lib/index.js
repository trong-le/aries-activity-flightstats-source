...
export default class FlightStatsSource extends Activity {
	...
	async fidsArrival(config) {
		const uri = `${baseURI}fids/rest/v1/json/CVG/arrivals?appId=${config.appId}&appKey={config.appKey}&requestedFields=${config.reqFields}&timeWindowBegin=${config.timeWindowBegin}&timeWindowEnd=${config.timeWindowEnd}&lateMinutes=${config.delayMinutes}`
		const params = {
			uri: uri,
			qs: config,
			json: true
	...
	async getFlightStatusArrivalsByAirport(config) {
		...
		const uri = `${baseURI}flightstatus/rest/v2/json/airport/status/${config.airport}/arr/${config.year}/${config.month}/${config.day}/${config.hour}?appId=${config.appId}&appKey=${config.appKey}&utc=${config.useUTC}`;
		const params = {
			uri: uri,
			qs: config,
			json: true
